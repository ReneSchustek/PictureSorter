...
/// </summary>
public sealed partial class SortViewModel : ObservableObject, IDisposable
{
        ...
        ILogger<SortViewModel> logger)
    {
        ...
        // Der Assistent kennt die Use-Cases nur über diese Delegaten (SRP).
    ...
    /// <summary>
    /// Die erzeugten Sortiervorschläge (Vorschau).
    /// </summary>
    public ObservableCollection<SortProposal> Proposals { get; } = [];
    ...
    /// <summary>
    /// Anwenden ist möglich, wenn Vorschläge in der Vorschau vorliegen.
    /// </summary>
    public bool CanApply => State is SortState.Preview && Proposals.Count > 0;
    ...
    private async Task ApplyAsync()
    {
        ...
        try
        {
            int moved = await _sorter
                .ApplyProposalsAsync(Proposals, dryRun: false, _cancellation.Token)
                .ConfigureAwait(true);
            ...
            _status.Finish($"{moved} Dateien sortiert.", StatusSeverity.Success);
        }
        catch (OperationCanceledException)
        {
            State = SortState.Preview;
